var S=Object.defineProperty;var B=(c,e,t)=>e in c?S(c,e,{enumerable:!0,configurable:!0,writable:!0,value:t}):c[e]=t;var f=(c,e,t)=>B(c,typeof e!="symbol"?e+"":e,t);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))i(a);new MutationObserver(a=>{for(const r of a)if(r.type==="childList")for(const o of r.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function t(a){const r={};return a.integrity&&(r.integrity=a.integrity),a.referrerPolicy&&(r.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?r.credentials="include":a.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function i(a){if(a.ep)return;a.ep=!0;const r=t(a);fetch(a.href,r)}})();window.addEventListener("online",()=>{document.getElementById("offline-indicator").classList.remove("show")});window.addEventListener("offline",()=>{document.getElementById("offline-indicator").classList.add("show")});navigator.onLine||document.getElementById("offline-indicator").classList.add("show");class m{static getUserData(){const e=localStorage.getItem("userData");return e?JSON.parse(e):null}static setUserData(e){localStorage.setItem("userData",JSON.stringify(e))}static getToken(){const e=this.getUserData();return e?e.token:null}static getUserId(){const e=this.getUserData();return e?e.userId:null}static getUserName(){const e=this.getUserData();return e?e.name:null}static isLoggedIn(){return!!this.getToken()}static logout(){localStorage.removeItem("userData")}}class y{constructor(){this._baseUrl="https://story-api.dicoding.dev/v1"}async getStories(e=1,t=10,i=0){try{const a=m.getToken();if(!a)throw new Error("Anda belum login");console.log(`Fetching stories: page=${e}, size=${t}, location=${i}`);const o=await(await fetch(`${this._baseUrl}/stories?page=${e}&size=${t}&location=${i}`,{headers:{Authorization:`Bearer ${a}`}})).json();if(o.error)throw new Error(o.message);return console.log("Stories fetched:",o.listStory.length),o.listStory}catch(a){throw console.error("Error getting stories:",a),new Error(a.message||"Gagal mengambil daftar cerita")}}async getStoryDetail(e){try{const t=m.getToken();if(!t)throw new Error("Anda belum login");console.log("Fetching story detail for ID:",e);const a=await(await fetch(`${this._baseUrl}/stories/${e}`,{headers:{Authorization:`Bearer ${t}`}})).json();if(a.error)throw new Error(a.message);return console.log("Story detail fetched"),a.story}catch(t){throw console.error("Error getting story detail:",t),new Error(t.message||"Gagal mengambil detail cerita")}}async addStory(e,t,i=null,a=null){try{const r=m.getToken(),o=new FormData;o.append("description",e),o.append("photo",t),i!==null&&a!==null&&(o.append("lat",i),o.append("lon",a));const s=r?`${this._baseUrl}/stories`:`${this._baseUrl}/stories/guest`,n=r?{Authorization:`Bearer ${r}`}:{};console.log("Adding new story",r?"with auth":"as guest");const d=await(await fetch(s,{method:"POST",headers:n,body:o})).json();if(d.error)throw new Error(d.message);return console.log("Story added successfully"),d}catch(r){throw console.error("Error adding story:",r),new Error(r.message||"Gagal menambahkan cerita")}}}class I{constructor(e,t){this._config=e,this._view=t}async getStories(){try{this._view.showLoading();const e=await this._config.getStories(1,10,1);this._view.renderStories(e)}catch(e){console.error("Home presenter error:",e),this._view.showError(e.message)}}}class h{static async openDB(){return new Promise((e,t)=>{if(!("indexedDB"in window)){t(new Error("Browser tidak mendukung IndexedDB"));return}const i=indexedDB.open(this.DB_NAME,this.DB_VERSION);i.onerror=a=>{console.error("IndexedDB error:",a.target.error),t(new Error("Gagal membuka database"))},i.onsuccess=()=>{e(i.result)},i.onupgradeneeded=a=>{const r=a.target.result;r.objectStoreNames.contains(this.STORE_STORIES)||(r.createObjectStore(this.STORE_STORIES,{keyPath:"id"}),console.log(`Object store ${this.STORE_STORIES} berhasil dibuat`)),r.objectStoreNames.contains(this.STORE_FAVORITES)||(r.createObjectStore(this.STORE_FAVORITES,{keyPath:"id"}),console.log(`Object store ${this.STORE_FAVORITES} berhasil dibuat`))}})}static async saveStories(e){const i=(await this.openDB()).transaction(this.STORE_STORIES,"readwrite"),a=i.objectStore(this.STORE_STORIES);return e.forEach(r=>{a.put(r)}),new Promise((r,o)=>{i.oncomplete=()=>{console.log("Stories berhasil disimpan ke IndexedDB"),r(e)},i.onerror=s=>{console.error("Error menyimpan stories:",s.target.error),o(new Error("Gagal menyimpan stories ke IndexedDB"))}})}static async getStories(){const a=(await this.openDB()).transaction(this.STORE_STORIES,"readonly").objectStore(this.STORE_STORIES).getAll();return new Promise((r,o)=>{a.onsuccess=()=>{r(a.result)},a.onerror=s=>{console.error("Error mengambil stories:",s.target.error),o(new Error("Gagal mengambil stories dari IndexedDB"))}})}static async getStoryById(e){const r=(await this.openDB()).transaction(this.STORE_STORIES,"readonly").objectStore(this.STORE_STORIES).get(e);return new Promise((o,s)=>{r.onsuccess=()=>{o(r.result)},r.onerror=n=>{console.error("Error mengambil story:",n.target.error),s(new Error("Gagal mengambil story dari IndexedDB"))}})}static async deleteStory(e){const r=(await this.openDB()).transaction(this.STORE_STORIES,"readwrite").objectStore(this.STORE_STORIES).delete(e);return new Promise((o,s)=>{r.onsuccess=()=>{console.log(`Story dengan id ${e} berhasil dihapus dari IndexedDB`),o(!0)},r.onerror=n=>{console.error("Error menghapus story:",n.target.error),s(new Error(`Gagal menghapus story dengan id ${e} dari IndexedDB`))}})}static async clearStories(){const a=(await this.openDB()).transaction(this.STORE_STORIES,"readwrite").objectStore(this.STORE_STORIES).clear();return new Promise((r,o)=>{a.onsuccess=()=>{console.log("Semua stories berhasil dihapus dari IndexedDB"),r(!0)},a.onerror=s=>{console.error("Error menghapus semua stories:",s.target.error),o(new Error("Gagal menghapus semua stories dari IndexedDB"))}})}static async addToFavorites(e){const a=(await this.openDB()).transaction(this.STORE_FAVORITES,"readwrite").objectStore(this.STORE_FAVORITES);return new Promise((r,o)=>{const s=a.put(e);s.onsuccess=()=>{console.log("Story berhasil ditambahkan ke favorit"),r(!0)},s.onerror=n=>{console.error("Error menambahkan favorit:",n.target.error),o(new Error("Gagal menambahkan story ke favorit"))}})}static async removeFromFavorites(e){const a=(await this.openDB()).transaction(this.STORE_FAVORITES,"readwrite").objectStore(this.STORE_FAVORITES);return new Promise((r,o)=>{const s=a.delete(e);s.onsuccess=()=>{console.log("Story berhasil dihapus dari favorit"),r(!0)},s.onerror=n=>{console.error("Error menghapus favorit:",n.target.error),o(new Error("Gagal menghapus story dari favorit"))}})}static async getFavorites(){const i=(await this.openDB()).transaction(this.STORE_FAVORITES,"readonly").objectStore(this.STORE_FAVORITES);return new Promise((a,r)=>{const o=i.getAll();o.onsuccess=()=>{a(o.result)},o.onerror=s=>{console.error("Error mengambil favorit:",s.target.error),r(new Error("Gagal mengambil daftar favorit"))}})}static async isFavorite(e){const a=(await this.openDB()).transaction(this.STORE_FAVORITES,"readonly").objectStore(this.STORE_FAVORITES);return new Promise((r,o)=>{const s=a.get(e);s.onsuccess=()=>{r(!!s.result)},s.onerror=n=>{console.error("Error memeriksa favorit:",n.target.error),o(new Error("Gagal memeriksa status favorit"))}})}}f(h,"DB_NAME","storyapps-db"),f(h,"DB_VERSION",1),f(h,"STORE_STORIES","stories"),f(h,"STORE_FAVORITES","favorites");class T{constructor(){this._config=new y,this._presenter=null,this._showFavorites=!1,this._title="StoryApps"}async render(){return document.title=this._title,`
      ...
      `;return}t.innerHTML="";for(const i of e){const a=i.name?i.name.charAt(0).toUpperCase():"?",r=await h.isFavorite(i.id),o=document.createElement("article");o.classList.add("story-card"),o.innerHTML=`
        ...
        <div class="story-content">
          ...
          <div class="story-actions">
            <button class="favorite-btn ${r?"favorited":""}" 
                    data-id="${i.id}"
                    data-tooltip="${r?"Klik untuk menghapus dari favorit":"Klik untuk menyimpan cerita"}"
                    data-label="${r?"Tersimpan":"Simpan"}">
              <i class="fa-${r?"solid":"regular"} fa-bookmark"></i>
            </button>
    ...
    `,document.getElementById("retry-button").addEventListener("click",async()=>{await this._presenter.getStories()}))}_truncateText(e,t){return e.length<=t?e:`${e.substr(0,t)}...`}_formatDate(e){const t={year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"};return new Date(e).toLocaleDateString("id-ID",t)}}class C{constructor(e,t){this._config=e,this._view=t}async addStory(e,t,i,a){try{if(this._view.clearAlert(),this._view.showLoading(),!e)throw new Error("Deskripsi cerita wajib diisi");if(!t)throw new Error("Foto wajib diambil");console.log("Adding story with",i&&a?"location":"no location");const r=await this._config.addStory(e,t,i,a);console.log("Story added successfully"),this._view.showAlert("Cerita berhasil dibagikan!","success"),setTimeout(()=>{window.location.href="#/"},1500)}catch(r){console.error("Add story presenter error:",r),this._view.hideLoading(),this._view.showAlert(r.message)}}}class P{constructor(){this._config=new y,this._presenter=null,this._map=null,this._marker=null,this._cameraStream=null,this._photoBlob=null,this._selectedLocation=null,this._photoSource=null,this._hashChangeHandler=null}async render(){return console.log("Rendering add story page"),`
    ...
    `;const i=document.getElementById("retry-button");i&&i.addEventListener("click",async()=>{await this._presenter.getStoriesWithLocation()})}_truncateText(e,t){return e.length<=t?e:e.substr(0,t)+"..."}beforeUnload(){this._map&&(this._map.remove(),this._map=null)}}class D{constructor(e,t){this._config=e,this._view=t}async getStoryDetail(e){try{this._view.showLoading();const t=await this._config.getStoryDetail(e);this._view.renderStoryDetail(t)}catch(t){console.error("Detail presenter error:",t),this._view.showError(t.message)}}}class O{constructor(){this._config=new y,this._presenter=null,this._map=null}async render(){return console.log("Rendering detail page"),`
    ...
    `}_formatDate(e){const t={year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"};return new Date(e).toLocaleDateString("id-ID",t)}_truncateText(e,t){return e.length<=t?e:e.substr(0,t)+"..."}beforeUnload(){this._map&&(this._map.remove(),this._map=null)}}class w{constructor(){this._baseUrl="https://story-api.dicoding.dev/v1"}async login(e,t){try{console.log("Trying to login with:",e);const a=await(await fetch(`${this._baseUrl}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:e,password:t})})).json();if(a.error)throw new Error(a.message);return console.log("Login successful:",a),a.loginResult}catch(i){throw console.error("Login error:",i),new Error(i.message||"Gagal melakukan login")}}async register(e,t,i){try{console.log("Registering new user:",t);const r=await(await fetch(`${this._baseUrl}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e,email:t,password:i})})).json();if(r.error)throw new Error(r.message);return console.log("Registration successful"),r}catch(a){throw console.error("Registration error:",a),new Error(a.message||"Gagal melakukan registrasi")}}async subscribeNotification(e){try{const t=m.getToken();if(!t)throw new Error("Anda belum login");const a=await(await fetch(`${this._baseUrl}/notifications/subscribe`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify(e)})).json();if(a.error)throw new Error(a.message);return a}catch(t){throw console.error("Notification subscription error:",t),new Error(t.message||"Gagal berlangganan notifikasi")}}async unsubscribeNotification(e){try{const t=m.getToken();if(!t)throw new Error("Anda belum login");const a=await(await fetch(`${this._baseUrl}/notifications/subscribe`,{method:"DELETE",headers:{"Content-Type":"application/json",Authorization:`Bearer ${t}`},body:JSON.stringify({endpoint:e})})).json();if(a.error)throw new Error(a.message);return a}catch(t){throw console.error("Unsubscribe error:",t),new Error(t.message||"Gagal berhenti berlangganan notifikasi")}}}const k=Object.freeze(Object.defineProperty({__proto__:null,AuthConfig:w},Symbol.toStringTag,{value:"Module"}));class R{constructor(e,t){this._config=e,this._view=t}async login(e,t){try{if(this._view.clearAlert(),this._view.showLoading(),!e||!t)throw new Error("Email dan password harus diisi");if(t.length<8)throw new Error("Password minimal 8 karakter");console.log("Login validation passed, calling API");const i=await this._config.login(e,t);console.log("Login successful, saving user data"),m.setUserData(i),console.log("Redirecting to home page"),window.location.href="#/",window.location.reload()}catch(i){console.error("Login presenter error:",i),this._view.hideLoading(),this._view.showAlert(i.message)}}}class x{constructor(){this._config=new w,this._presenter=null}async render(){return console.log("Rendering login page"),`
    ...
    `}async afterRender(){console.log("Login page afterRender"),this._presenter=new R(this._config,this);const e=document.getElementById("login-form");if(!e){console.error("Login form not found in DOM");return}e.addEventListener("submit",async t=>{t.preventDefault();const i=document.getElementById("email").value,a=document.getElementById("password").value;await this._presenter.login(i,a)})}showLoading(){const e=document.querySelector('#login-form button[type="submit"]');e&&(e.innerHTML='<i class="fas fa-spinner fa-spin"></i> Loading...',e.disabled=!0)}hideLoading(){const e=document.querySelector('#login-form button[type="submit"]');e&&(e.innerHTML='<i class="fas fa-sign-in-alt"></i> Login',e.disabled=!1)}showAlert(e,t="danger"){const i=document.getElementById("alert-container");i&&(i.innerHTML=`
        <div class="alert alert-${t}">
          ${e}
        </div>
      `,i.scrollIntoView({behavior:"smooth"}))}clearAlert(){const e=document.getElementById("alert-container");e&&(e.innerHTML="")}}class ${constructor(e,t){this._config=e,this._view=t}async register(e,t,i,a){try{if(this._view.clearAlert(),this._view.showLoading(),!e||!t||!i||!a)throw new Error("Semua field harus diisi");if(i.length<8)throw new Error("Password minimal 8 karakter");if(i!==a)throw new Error("Password dan konfirmasi password tidak cocok");console.log("Register validation passed, calling API"),await this._config.register(e,t,i),console.log("Registration successful"),this._view.showAlert("Registrasi berhasil. Silakan login.","success"),setTimeout(()=>{window.location.href="#/login"},2e3)}catch(r){console.error("Register presenter error:",r),this._view.hideLoading(),r.message==="Email is already taken"?this._view.showAlert("Email sudah terdaftar, silakan gunakan email lain atau login."):this._view.showAlert(r.message)}}}class N{constructor(){this._config=new w,this._presenter=null}async render(){return console.log("Rendering register page"),`
    ...
    `}async afterRender(){console.log("Register page afterRender"),this._presenter=new $(this._config,this);const e=document.getElementById("register-form");if(!e){console.error("Register form not found in DOM");return}e.addEventListener("submit",async t=>{t.preventDefault();const i=document.getElementById("name").value,a=document.getElementById("email").value,r=document.getElementById("password").value,o=document.getElementById("confirmPassword").value;await this._presenter.register(i,a,r,o)})}showLoading(){const e=document.querySelector('#register-form button[type="submit"]');e&&(e.innerHTML='<i class="fas fa-spinner fa-spin"></i> Loading...',e.disabled=!0)}hideLoading(){const e=document.querySelector('#register-form button[type="submit"]');e&&(e.innerHTML='<i class="fas fa-user-plus"></i> Daftar',e.disabled=!1)}showAlert(e,t="danger"){const i=document.getElementById("alert-container");i&&(i.innerHTML=`
        <div class="alert alert-${t}">
          ${e}
        </div>
      `,i.scrollIntoView({behavior:"smooth"}))}clearAlert(){const e=document.getElementById("alert-container");e&&(e.innerHTML="")}}class U{constructor(){this._title="halaman tidak ditemukan - StoryApps"}async render(){return document.title=this._title,`
    ...
    `}async afterRender(){const e=document.querySelector(".not-found-content");e&&e.classList.add("animate-fade-in")}}class F{constructor(){this._title="StoryApps"}async render(){return document.title=this._title,`
    ...
    `}async afterRender(){if(!m.isLoggedIn()){window.location.href="#/login";return}this.showLoading();try{const e=await h.getFavorites();this.renderFavorites(e)}catch(e){console.error("Error loading favorites:",e),this.showError("Gagal memuat cerita favorit: "+e.message)}}showLoading(){const e=document.getElementById("favorites-loader");e&&e.classList.remove("hidden");const t=document.getElementById("error-container");t&&t.classList.add("hidden")}hideLoading(){const e=document.getElementById("favorites-loader");e&&e.classList.add("hidden")}async renderFavorites(e){this.hideLoading();const t=document.getElementById("favorites-container");if(!t){console.error("Favorites container not found");return}if(!e||e.length===0){t.innerHTML=`
      ...
      `;return}t.innerHTML="",e.forEach(i=>{const a=i.name.charAt(0).toUpperCase(),r=document.createElement("article");r.classList.add("story-card"),r.innerHTML=`
        <div class="story-image-container">
          <img
            src="${i.photoUrl}"
            alt="Cerita dari ${i.name}"
      ...
      `,t.appendChild(r),r.querySelector(".view-details-btn").addEventListener("click",n=>{n.preventDefault(),window.selectedStoryId=i.id,window.location.href="#/detail"}),r.querySelector(".favorite-btn").addEventListener("click",async n=>{n.preventDefault();try{await h.removeFromFavorites(i.id),r.remove(),(await h.getFavorites()).length===0&&this.renderFavorites([]),"Notification"in window&&Notification.permission==="granted"&&navigator.serviceWorker.ready.then(d=>{d.showNotification("StoryApps",{body:"Cerita berhasil dihapus dari favorit",icon:"./src/public/icons/icon-192x192.png",badge:"./src/public/icons/badge-96x96.png",vibrate:[100,50,100]})})}catch(l){console.error("Error removing from favorites:",l)}})})}showError(e){this.hideLoading();const t=document.getElementById("error-container");if(!t){console.error("Error container not found");return}t.classList.remove("hidden"),t.innerHTML=`
      <div class="error-content">
        <i class="fas fa-exclamation-triangle fa-3x"></i>
    ...
    `;const i=document.getElementById("retry-button");i&&i.addEventListener("click",async()=>{this.showLoading();try{const a=await h.getFavorites();this.renderFavorites(a)}catch(a){console.error("Error reloading favorites:",a),this.showError("Gagal memuat cerita favorit: "+a.message)}})}_truncateText(e,t){return e?e.length<=t?e:e.substr(0,t)+"...":""}_formatDate(e){if(!e)return"-";const t={year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"};return new Date(e).toLocaleDateString("id-ID",t)}}const H="modulepreload",G=function(c){return"/"+c},_={},E=function(e,t,i){let a=Promise.resolve();if(t&&t.length>0){document.getElementsByTagName("link");const o=document.querySelector("meta[property=csp-nonce]"),s=(o==null?void 0:o.nonce)||(o==null?void 0:o.getAttribute("nonce"));a=Promise.allSettled(t.map(n=>{if(n=G(n),n in _)return;_[n]=!0;const l=n.endsWith(".css"),d=l?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${n}"]${d}`))return;const u=document.createElement("link");if(u.rel=l?"stylesheet":H,l||(u.as="script"),u.crossOrigin="",u.href=n,s&&u.setAttribute("nonce",s),document.head.appendChild(u),l)return new Promise((v,b)=>{u.addEventListener("load",v),u.addEventListener("error",()=>b(new Error(`Unable to preload CSS for ${n}`)))})}))}function r(o){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=o,window.dispatchEvent(s),!s.defaultPrevented)throw o}return a.then(o=>{for(const s of o||[])s.status==="rejected"&&r(s.reason);return e().catch(r)})};class p{static async registerServiceWorker(){if(!("serviceWorker"in navigator))return console.log("Service Worker tidak didukung di browser ini"),null;try{const e=await navigator.serviceWorker.register("./sw.js",{scope:"./"});return console.log("Service Worker berhasil didaftarkan",e),e}catch(e){return console.error("Registrasi Service Worker gagal:",e),null}}static async requestPermission(){if(!("Notification"in window))return console.log("Browser tidak mendukung notifikasi"),!1;const e=await Notification.requestPermission();return e==="denied"?(console.log("Fitur notifikasi tidak diizinkan"),!1):e==="default"?(console.log("Pengguna menutup kotak dialog permintaan izin"),!1):!0}static async getVapidPublicKey(){return"BCCs2eonMI-6H2ctvFaWg-UYdDv387Vno_bzUzALpB442r2lCnsHmtrx8biyPi_E-1fSGABK_Qs_GlvPoJJqxbk"}static async subscribePushNotification(e){if(!e.active){console.error("Service Worker tidak aktif");return}const t=await this.getVapidPublicKey(),i=this._urlBase64ToUint8Array(t);try{const a=await e.pushManager.subscribe({userVisibleOnly:!0,applicationServerKey:i});return console.log("Berhasil melakukan subscribe dengan endpoint:",a.endpoint),await this._sendSubscriptionToServer(a),a}catch(a){return console.error("Gagal melakukan subscribe:",a),null}}static async _sendSubscriptionToServer(e){try{const{AuthConfig:t}=await E(async()=>{const{AuthConfig:s}=await Promise.resolve().then(()=>k);return{AuthConfig:s}},void 0),i=new t,a=btoa(String.fromCharCode(...new Uint8Array(e.getKey("p256dh")))),r=btoa(String.fromCharCode(...new Uint8Array(e.getKey("auth")))),o={endpoint:e.endpoint,keys:{p256dh:a,auth:r}};return await i.subscribeNotification(o),console.log("Berhasil mengirim subscription ke server"),o}catch(t){throw console.error("Gagal mengirim subscription ke server:",t),t}}static async unsubscribePushNotification(e){try{const t=await e.pushManager.getSubscription();if(!t)return console.log("Tidak ada subscription yang aktif"),!1;const{AuthConfig:i}=await E(async()=>{const{AuthConfig:o}=await Promise.resolve().then(()=>k);return{AuthConfig:o}},void 0);await new i().unsubscribeNotification(t.endpoint);const r=await t.unsubscribe();return console.log(r?"Berhasil unsubscribe dari push notification":"Gagal unsubscribe dari push notification"),r}catch(t){return console.error("Error unsubscribing from push notification:",t),!1}}static async getSubscriptionStatus(e){try{return await e.pushManager.getSubscription()!==null}catch(t){return console.error("Error checking subscription status:",t),!1}}static _urlBase64ToUint8Array(e){const t="=".repeat((4-e.length%4)%4),i=(e+t).replace(/-/g,"+").replace(/_/g,"/"),a=window.atob(i),r=new Uint8Array(a.length);for(let o=0;o<a.length;o++)r[o]=a.charCodeAt(o);return r}static showNotification(e,t){if(!("Notification"in window)){console.log("Browser tidak mendukung notifikasi");return}Notification.permission==="granted"?navigator.serviceWorker.ready.then(i=>{i.showNotification(e,t)}):(console.log("Izin notifikasi tidak diberikan"),alert("Anda belum memberikan izin untuk menerima notifikasi."))}static async testNotification(){if(!("Notification"in window))return console.log("Browser tidak mendukung notifikasi"),!1;if(Notification.permission!=="granted")return console.log("Izin notifikasi tidak diberikan"),!1;try{return await(await navigator.serviceWorker.ready).showNotification("Test Notification",{body:"Ini adalah notifikasi test dari StoryApps",icon:"./src/public/icons/icon-192x192.png",badge:"./src/public/icons/badge-96x96.png",vibrate:[100,50,100],tag:"test-notification"}),console.log("Test notification sent successfully"),!0}catch(e){return console.error("Error sending test notification:",e),!1}}static async testPushNotification(){try{const t=await(await navigator.serviceWorker.ready).pushManager.getSubscription();if(!t)return console.log("No push subscription found"),!1;const i="Test push notification from StoryApps",a=new Blob([i],{type:"text/plain"});return console.log("Push subscription details:",{endpoint:t.endpoint,keys:{p256dh:btoa(String.fromCharCode(...new Uint8Array(t.getKey("p256dh")))),auth:btoa(String.fromCharCode(...new Uint8Array(t.getKey("auth"))))}}),console.log("Push subscription is ready for testing"),!0}catch(e){return console.error("Error testing push notification:",e),!1}}}class q{constructor(){this._title="Pengaturan Notifikasi - StoryApps",this._isSubscribed=!1,this._registration=null}async render(){return document.title=this._title,`
        ...
        `,document.body.appendChild(t),setTimeout(()=>{t.remove()},3e3)}_showErrorMessage(e){const t=document.createElement("div");t.className="notification-error",t.innerHTML=`
            ...
            <span>${e}</span>
        `,document.body.appendChild(t),setTimeout(()=>{t.remove()},5e3)}beforeUnload(){}}class z{constructor(){this._title="Offline - StoryApps"}async render(){return document.title=this._title,`
        ...
        `}async afterRender(){this._bindEvents(),this._checkConnectionStatus()}_bindEvents(){const e=document.getElementById("retry-connection");e&&e.addEventListener("click",()=>{this._retryConnection()}),window.addEventListener("online",()=>{this._handleOnline()}),window.addEventListener("offline",()=>{this._handleOffline()})}_retryConnection(){const e=document.getElementById("retry-connection");e.disabled=!0,e.innerHTML='<div class="loader"></div> Memeriksa...',setTimeout(()=>{navigator.onLine?this._handleOnline():(e.disabled=!1,e.innerHTML='<i class="fas fa-sync-alt"></i> Coba Lagi',this._showMessage("Koneksi masih tidak tersedia","error"))},2e3)}_handleOnline(){this._showMessage("Koneksi tersedia! Mengalihkan...","success"),setTimeout(()=>{window.location.href="#/",window.location.reload()},1500)}_handleOffline(){this._showMessage("Koneksi terputus","error")}_checkConnectionStatus(){navigator.onLine&&this._handleOnline()}_showMessage(e,t="info"){const i=document.querySelector(".connection-message");i&&i.remove();const a=document.createElement("div");a.className=`connection-message connection-message-${t}`,a.innerHTML=`
            ...
            <span>${e}</span>
        `;const r=document.querySelector(".offline-content");r.insertBefore(a,r.firstChild),setTimeout(()=>{a.parentNode&&a.parentNode.removeChild(a)},3e3)}beforeUnload(){window.removeEventListener("online",this._handleOnline),window.removeEventListener("offline",this._handleOffline)}}const g={"/":{view:T},"/add":{view:P},"/map":{view:A},"/detail":{view:O},"/login":{view:x},"/register":{view:N},"/favorites":{view:F},"/notifications":{view:q},"/offline":{view:z},"/404":{view:U}};class j{static parseActiveUrlWithCombiner(){const e=window.location.hash.slice(1).toLowerCase();return this._urlCombiner(this._urlSplitter(e))}static parseActiveUrlWithoutCombiner(){const e=window.location.hash.slice(1).toLowerCase();return this._urlSplitter(e)}static _urlSplitter(e){const t=e.split("/");return{resource:t[1]||null,id:t[2]||null,verb:t[3]||null}}static _urlCombiner(e){return e.resource?`/${e.resource}`:"/"}}class W{static init(){this.deferredPrompt=null,this._setupEventListeners()}static _setupEventListeners(){window.addEventListener("beforeinstallprompt",e=>{e.preventDefault(),this.deferredPrompt=e,this._showInstallBanner()}),document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("install-pwa"),t=document.getElementById("close-banner");e&&e.addEventListener("click",function(){window.location.href="https://storyaps.netlify.app/"}),t&&t.addEventListener("click",()=>{this._hideInstallBanner()})}),window.addEventListener("appinstalled",()=>{this._hideInstallBanner(),this.deferredPrompt=null,console.log("PWA was installed")})}static _showInstallBanner(){const e=document.getElementById("pwa-install-banner");e&&this.deferredPrompt&&e.classList.add("show")}static _hideInstallBanner(){const e=document.getElementById("pwa-install-banner");e&&e.classList.remove("show")}static async _installPwa(){if(!this.deferredPrompt)return;this.deferredPrompt.prompt();const e=await this.deferredPrompt.userChoice;this.deferredPrompt=null,e.outcome==="accepted"?console.log("User accepted the install prompt"):console.log("User dismissed the install prompt"),this._hideInstallBanner()}}window.selectedStoryId=null;window.routes=g;class V{constructor(){this._currentPage=null,this._initializeApp()}async _initializeApp(){console.log("Initializing app..."),await this._initIndexedDB(),await this._initServiceWorker(),W.init(),await this._subscribeToPushNotification(),this._checkAuthStatus(),await this._handleRoute(),window.addEventListener("hashchange",()=>{this._cleanupCurrentPage(),this._handleRoute()}),window.addEventListener("beforeunload",()=>{this._cleanupCurrentPage()}),document.addEventListener("click",e=>{e.target.tagName==="A"&&e.target.href.includes("#/")&&document.startViewTransition&&(e.preventDefault(),document.startViewTransition(()=>{window.location.href=e.target.href}))})}async _initServiceWorker(){try{if(console.log("Initializing Service Worker..."),"serviceWorker"in navigator){const e=await navigator.serviceWorker.register("/sw.js",{scope:"/"});return console.log("Service Worker registered successfully:",e),e.addEventListener("updatefound",()=>{console.log("New service worker found, updating...")}),m.isLoggedIn()&&await p.requestPermission()&&e&&await p.subscribePushNotification(e),e}else return console.warn("Service Worker not supported"),null}catch(e){return console.error("Error initializing service worker:",e),null}}async _initIndexedDB(){try{await h.openDB(),console.log("IndexedDB initialized successfully")}catch(e){console.error("Failed to initialize IndexedDB:",e)}}_checkAuthStatus(){var l,d,u,v;console.log("Checking auth status...");const e=m.isLoggedIn(),t=document.getElementById("login-menu"),i=document.getElementById("register-menu"),a=document.getElementById("logout-menu"),r=(l=document.querySelector('.nav-item a[href="#/favorites"]'))==null?void 0:l.parentElement,o=(d=document.querySelector('.nav-item a[href="#/add"]'))==null?void 0:d.parentElement,s=(u=document.querySelector('.nav-item a[href="#/map"]'))==null?void 0:u.parentElement,n=(v=document.querySelector('.nav-item a[href="#/notifications"]'))==null?void 0:v.parentElement;if(!t||!i||!a){console.error("Menu items not found");return}e?(console.log("User is logged in"),t.classList.add("hidden"),i.classList.add("hidden"),a.classList.remove("hidden"),r&&r.classList.remove("hidden"),o&&o.classList.remove("hidden"),s&&s.classList.remove("hidden"),n&&n.classList.remove("hidden"),this._subscribeToPushNotification()):(console.log("User is not logged in"),t.classList.remove("hidden"),i.classList.remove("hidden"),a.classList.add("hidden"),r&&r.classList.add("hidden"),o&&o.classList.add("hidden"),s&&s.classList.add("hidden"),n&&n.classList.add("hidden")),a.addEventListener("click",b=>{b.preventDefault(),this._handleLogout()})}async _subscribeToPushNotification(){try{if("serviceWorker"in navigator){const e=await navigator.serviceWorker.ready;await p.requestPermission()&&e&&await p.subscribePushNotification(e)}}catch(e){console.error("Error subscribing to push notification:",e)}}async _handleLogout(){this._cleanupCurrentPage(),m.logout();try{await h.clearStories(),console.log("Stories cleared from IndexedDB after logout")}catch(e){console.error("Error clearing stories from IndexedDB:",e)}window.location.href="#/",window.location.reload()}_cleanupCurrentPage(){this._currentPage&&typeof this._currentPage.beforeUnload=="function"&&(console.log("Cleaning up current page..."),this._currentPage.beforeUnload(),this._currentPage=null)}async _handleRoute(){console.log("Handling route..."),console.log("Current hash:",window.location.hash),this._cleanupCurrentPage();const e=window.location.hash.slice(1).split("/");e.length>2&&e[1]==="detail"&&(window.selectedStoryId=e[2],window.history.replaceState(null,null,"#/detail"));const t=j.parseActiveUrlWithCombiner();console.log("Parsed URL:",t),console.log("Available routes:",Object.keys(g)),console.log("Current URL:",t);let i;g[t]?i=g[t]:(console.log("Route not found, redirecting to 404 page"),i=g["/404"]),console.log("Page to render:",i);try{if((t==="/login"||t==="/register")&&m.isLoggedIn()){console.log("User is logged in, redirecting to home"),window.location.href="#/";return}if((t==="/add"||t==="/map"||t==="/favorites"||t==="/notifications")&&!m.isLoggedIn()){console.log("Protected route, redirecting to login"),window.location.href="#/login";return}const a=document.querySelector("#content");if(!a){console.error("Content container not found");return}a.innerHTML="",this._currentPage=new i.view,console.log("View instantiated");const r=await this._currentPage.render();console.log("Content rendered"),a.innerHTML=r,console.log("Content injected into DOM"),await this._currentPage.afterRender(),console.log("afterRender completed"),document.getElementById("main-content").focus()}catch(a){console.error("Error rendering page:",a)}}}document.addEventListener("DOMContentLoaded",()=>{console.log("DOM fully loaded"),window.app=new V,console.log("App initialized:",window.app),console.log("Routes available:",Object.keys(g))});
